...
2. OpenAPI와 스웨거를 활용한 실전 API 설계 (진행중)
   - API를 개발하면서 클라이언트 개발자와의 협업이나 API의 문서화, 버전관리, API의 경계와 설계에 대한 노하우가 더 필요하다고 느꼈다.
   - 이 책을 읽으면서 좀 더 스웨거를 이용해 API 정보를 풍부하게 제공하고, 효율적인 API 개발에 대한 인사이트를 어느정도 배우고 있다
